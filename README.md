...
## Длинные посты — цепочка ответов

Если пост не вмещается в один message, раздели `message.md` на части с помощью `---split---`:
...
Каждая часть отправляется как ответ на предыдущую, образуя цепочку.
Части автоматически нумеруются: `[1/3]`, `[2/3]`, `[3/3]`.
Медиа прикрепляется только к первой части.
